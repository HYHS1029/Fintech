...
def buildLSTM(shape):
  model = Sequential()
  model.add(LSTM(9, input_shape = (shape[1], shape[2])))
...
lstm = buildLSTM(X_train.shape)
#%% LSTM result
LSTM_history = lstm.fit(X_train, Y_train, validation_data=(X_val, Y_val), batch_size=16, epochs=20)
y_hat = lstm.predict(X_val)
plt.figure()
